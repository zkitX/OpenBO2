...
bdSocketStatusCode bdPlatformSocket::bind(int* handle, const sockaddr_in addr, const unsigned __int16 port)
{
    ...
    *(int*)localAddr.sin_zero = 0;
    *(int*)&localAddr.sin_zero[4] = 0;
    localAddr.sin_family = 2;
    localAddr.sin_addr = addr.sin_addr;
    v4 = htons(port);
    v5 = handle;
    localAddr.sin_port = v4;
    if (bind(v5, localAddr, 16) != BD_NET_ERROR)
...
int bdPlatformSocket::sendTo(const int handle, const in_addr addr, const unsigned __int16 port, const void* data, const unsigned int length)
{
    int result; // eax
    int v6; // eax
    int v7; // ecx
...
int bdPlatformSocket::receiveFrom(const int handle, in_addr* addr, unsigned __int16* port, void* data, const unsigned int size)
{
    int result; // eax
    int v6; // esi
    int lastError; // eax
...
unsigned int bdPlatformSocket::getHostByName(const char* const name, in_addr* adresses, unsigned int numAdresses)
{
