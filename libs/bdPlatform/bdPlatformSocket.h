...
public:
	...
	bdSocketStatusCode bind(int* handle, const sockaddr_in addr, const unsigned __int16 port);
	int sendTo(const int handle, const in_addr addr, const unsigned __int16 port, const void* data, const unsigned int length);
	int receiveFrom(const int handle, in_addr* addr, unsigned __int16* port, void* data, const unsigned int size);
	bool close(int* handle);
	unsigned int getHostByName(const char* const name, in_addr* adresses, unsigned int numAdresses);
};